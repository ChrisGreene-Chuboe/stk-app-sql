...
----PARTITION_GUIDE: Step 1 - Add primary table above this line for FK references
CREATE TABLE private.stk_changeme ( ----PARTITION_GUIDE: Step 2 - Rename to stk_changeme_part
  uu UUID PRIMARY KEY DEFAULT gen_random_uuid(), ----PARTITION_GUIDE: Step 3 - Change to: uu UUID NOT NULL REFERENCES private.stk_changeme(uu),
  ...
  search_key TEXT NOT NULL UNIQUE DEFAULT gen_random_uuid(), ----PARTITION_GUIDE: Step 4 - Remove UNIQUE constraint
  name TEXT NOT NULL,
  description TEXT
  ----PARTITION_GUIDE: Step 5 - Add composite primary key: , primary key (uu, PARTITION_COLUMN)
); ----PARTITION_GUIDE: Step 6 - Add partition clause: PARTITION BY LIST|RANGE|HASH (PARTITION_COLUMN);
COMMENT ON TABLE private.stk_changeme IS 'Holds changeme records'; ----PARTITION_GUIDE: Update table name to stk_changeme_part

----PARTITION_GUIDE: Step 7 - Add default partition after comment

CREATE VIEW api.stk_changeme AS SELECT * FROM private.stk_changeme; ----PARTITION_GUIDE: Step 8 - Replace with join view
COMMENT ON VIEW api.stk_changeme IS 'Holds changeme records';

----PARTITION_GUIDE: Step 9 - Add partition triggers after view
---- primary_section end ----

-- create triggers for newly created tables
SELECT private.stk_trigger_create();

----PARTITION_OPTIONS: Common partitioning strategies
-- BY LIST (type_uu) - Good for enum-based separation
-- BY LIST (stk_entity_uu) - Good for multi-tenant isolation  
-- BY RANGE (created) - Good for time-based archival
-- BY HASH (uu) - Good for even distribution
-- BY LIST (table_name_uu_json->>'table_name') - Good for reference-based separation