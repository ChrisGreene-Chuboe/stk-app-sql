...
const STK_INVOICE_COLUMNS = [name, description, is_template, is_valid, record_json]
const STK_INVOICE_LINE_COLUMNS = [name, description, is_template, is_valid, record_json]
...
# Create a new invoice with specified name and type
#
...
#   bp list | where name == "ACME Corp" | invoice new "January Services" --description "Monthly consulting services"
#   bp list | first | invoice new "Q1-2024 Invoice" --type-search-key "SALES_STANDARD"
#   $customer_uuid | invoice new "Deposit Invoice" --type-search-key "SALES_DEPOSIT" --description "50% project deposit"
#   $vendor_uuid | invoice new "Office Supplies" --type-search-key "PURCHASE_STANDARD"
#   $bp_uuid | invoice new "Invoice Template" --template --json '{"payment_terms": "Net 30", "tax_rate": 0.08}'
#   
#   # Interactive examples:
#   $bp_uuid | invoice new "March Invoice" --type-search-key SALES_STANDARD --interactive
#   bp list | first | invoice new "Vendor Bill" --interactive --description "Equipment purchase"
#
# Returns: The UUID and name of the newly created invoice record
# Note: Uses chuck-stack conventions for automatic entity and type assignment
export def "invoice new" [
    name: string                    # The name/number of the invoice to create
...
#   invoice list | select name description | table
#   invoice list | where name =~ "INV"
...
#   def il [] { invoice list | lines | select name description lines }  # Concise invoice view with lines
#
# Using elaborate to resolve foreign key references:
#   invoice list | elaborate                                          # Resolve with default columns
#   invoice list | elaborate name stk_business_partner_uu             # Show business partner details
#   invoice list | elaborate --detail | select name stk_business_partner_uu_resolved.name  # BP names
#
...
#   invoice list | where name == "INV-2024-001" | invoice get
...
#
# Examples:
#   invoice list | where name == "cancelled-invoice" | get uu.0 | invoice revoke
#   invoice list | where name == "cancelled-invoice" | invoice revoke
...
# Add a line item to an invoice with specified name and type
#
...
#
# Examples:
#   $invoice_uuid | invoice line new "Consulting Services"
#   invoice list | where name == "INV-2024-001" | invoice line new "Software License" --description "Annual license fee" --type-search-key "ITEM"
#   invoice list | first | invoice line new "Setup Fee" --description "One-time setup fee"
#   $invoice_uuid | invoice line new "10% Discount" --type-search-key "DISCOUNT" --description "Early payment discount"
#   $invoice_uuid | invoice line new "Implementation Services" --json '{"quantity": 40, "unit_price": 150, "total": 6000}'
#   
#   # Interactive examples:
#   $invoice_uuid | invoice line new "Product Item" --type-search-key ITEM --interactive
#   invoice list | first | invoice line new "Professional Services" --interactive
#
# Returns: The UUID and name of the newly created invoice line record
# Note: Uses chuck-stack conventions for automatic entity and type assignment
export def "invoice line new" [
    name: string                    # The name of the invoice line
...
#   $invoice_uuid | invoice line list | where name == "Software License" | invoice line get
...
#
# Examples:
#   $invoice_uuid | invoice line list | where name == "obsolete-item" | get uu.0 | invoice line revoke
#   $invoice_uuid | invoice line list | where name == "obsolete-item" | invoice line revoke
