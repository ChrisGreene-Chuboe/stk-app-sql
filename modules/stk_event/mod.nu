...
export def "event list" [] {
    psql exec "SELECT uu, name, record_json, created FROM api.stk_event ORDER BY created DESC LIMIT 10"
}
...
export def "event get" [
    ...
    psql exec $"SELECT uu, name, record_json, created FROM api.stk_event WHERE uu = '($uu)'"
}
