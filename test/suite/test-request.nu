...
assert ($created | describe | str starts-with "record") "Should create request"
...
let get_record = ($parent | request get)
...
let revoke_string = ($revoke_item.uu | request revoke)
...
let revoke_param = (request revoke --uu $revoke_item2.uu)
...
let revoke_record = ($revoke_item3 | request revoke)
...
let json_detail = ($json_created.uu | request get)
...
let no_json_detail = ($no_json.uu | request get)
...
let complex_detail = ($complex_created.uu | request get)
...
assert ($attached_request | describe | str starts-with "record") "Should create attached request"
let attached_detail = ($attached_request.uu | request get)
...
assert ($param_attached | describe | str starts-with "record") "Should create request with --attach"
let param_detail = ($param_attached.uu | request get)
...
let process_result = (request process $process_request.uu)
...
let child_request = ($parent_request.uu | .append request $"Child Task($test_suffix)" --description "Subtask of parent")
assert ($child_request | describe | str starts-with "record") "Should create child request"
let child_detail = ($child_request.uu | request get)
...
let request_event = ($request_for_event.uu | .append event $"status-changed($test_suffix)" --description "Request status updated")
assert ($request_event | describe | str starts-with "record") "Should create event"
