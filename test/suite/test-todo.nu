...
assert (($created | describe | str starts-with "record")) "Should create todo"
...
let get_record = ($parent | todo get)
...
let revoke_string = ($revoke_item.uu | todo revoke)
...
let revoke_param = (todo revoke --uu $revoke_item2.uu)
...
let revoke_record = ($revoke_item3 | todo revoke)
...
let json_detail = ($json_created.uu | todo get)
...
let no_json_detail = ($no_json.uu | todo get)
...
let complex_detail = ($complex_created.uu | todo get)
...
let child_todo = ($parent_todo.uu | todo new $"Child Task($test_suffix)" --description "Subtask")
assert (($child_todo | describe | str starts-with "record")) "Should create child todo"
let child_detail = ($child_todo.uu | todo get)
...
let described_detail = ($described.uu | todo get)
...
let todo_event = ($todo_for_event.uu | .append event $"status-changed($test_suffix)" --description "Todo completed")
assert (($todo_event | describe | str starts-with "record")) "Should create event"
...
let todo_request = ($todo_for_request.uu | .append request $"follow-up($test_suffix)" --description "Additional work needed")
assert (($todo_request | describe | str starts-with "record")) "Should create request"
