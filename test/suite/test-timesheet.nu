...
assert ($created | describe | str starts-with "record") "Should create timesheet"
assert ($created.uu | is-not-empty) "Should have UUID"
# Name format may vary - just verify it exists
assert ($created.name | is-not-empty) "Should have name"
...
let get_record = ($parent | timesheet get)
...
let revoke_string = ($revoke_item.uu | timesheet revoke)
...
let revoke_param = (timesheet revoke --uu $revoke_item2.uu)
...
let revoke_record = ($revoke_item3 | timesheet revoke)
...
let auto_json_detail = ($auto_json.uu | timesheet get)
...
assert ($hours_timesheet | describe | str starts-with "record") "Should create timesheet with hours"
let hours_detail = ($hours_timesheet.uu | timesheet get)
...
assert ($dated_timesheet | describe | str starts-with "record") "Should create timesheet with custom date"
let dated_detail = ($dated_timesheet.uu | timesheet get)
...
let described_detail = ($described.uu | timesheet get)
...
let timesheet_event = ($timesheet_for_event.uu | .append event $"review-complete($test_suffix)" --description "Manager approved")
assert ($timesheet_event | describe | str starts-with "record") "Should create event"
...
let timesheet_request = ($timesheet_for_request.uu | .append request $"approve-overtime($test_suffix)" --description "Please approve overtime")
assert ($timesheet_request | describe | str starts-with "record") "Should create request"
